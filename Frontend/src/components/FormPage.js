...
function FormPage() {
  const [outputType, setOutputType] = useState("Standard");
  ...
  const handleSubmit = async (event) => {
    ...
    var endpoint = !description
      ? "http://127.0.0.1:5000/generate_mcqs"
      : "http://127.0.0.1:5000/generate_mcqs_with_description";
    ...
    try {
      ...
      navigate("/mcq-results", {
        state: {
          ...
          hasDescription: !!description, // Pass a boolean indicating if description exists
        ...
        <div className="px-4 py-5 sm:p-6">
          ...
          <form className="space-y-6" onSubmit={handleSubmit}>
            ...
            <div>
              ...
              <div className="flex space-x-4 tooltip">
                {["Standard", "Creative"].map((type) => (
                  <button
                    key={type}
                    type="button"
                ...
                <span className="tooltiptext">
                  If you don't like the questions generated in Standard Mode,
                  please try the Creative Mode, but it's a more experimental
                  feature
                </span>
              </div>
            </div>
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">
                Choose Cognitive Level (Bloom's Taxonomy)
              </label>
              <div className="flex space-x-4">
                {["Remember", "Understand", "Apply"].map((level) => (
                  <button
                    key={level}
                    type="button"
                    onClick={() => handleCognitiveLevelChange(level)}
                    className={`px-4 py-2 rounded-md ${
